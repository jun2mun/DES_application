...
def socket_func(sock,conn): # deprecated
    global process_on
    while 1:
                ...
                if is_camera == False:
                    ...
                    if camera_loaded:    
                        if ISAVL:
                            ...
                            sendTextViaSocket(f'detected,{cur_cnt}',conn)
                        else:
                            #print('ISAVL not detected') # 출력 안됨
                            sendTextViaSocket(f'not detected,{cur_cnt}',conn)
                    else:
...
def sendTextViaSocket(message, sock): #deprecated
...
if __name__ == '__main__': 
    ...
    t3 = threading.Thread(target=winapi_Test.foreground_handler)
    #t1.start()
    t2.start()
    t3.start()