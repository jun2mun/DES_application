...
normal_raw_data = pd.read_csv("./data/renewal/main.csv")
user_mean = normal_raw_data.mean()['count']
abnormal_raw_data = pd.read_csv("./data/renewal/professor.csv")

normal_data_list = normal_raw_data['count'].values.tolist()

## 전처리
normal_data = []; abnormal_data = [] ; seq_len = 4
for i in range(len(normal_data_list) - seq_len):
    if (user_mean-9 > normal_data_list[(i+seq_len)] or normal_data_list[(i+seq_len)] > user_mean+9)  :
